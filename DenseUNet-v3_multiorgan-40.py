...
training_x=training_x.astype('uint16')
training_y=training_y.astype('int8')
...
checkpoint = ModelCheckpoint(log_path+'/d-unet_model_Eloss.h5', verbose=1, 
                             monitor='val_loss',save_best_only=True, mode='auto')
checkpoint2 = ModelCheckpoint(log_path+'/d-unet_model_Epoch{epoch:04d}.h5', verbose=1,
                              save_freq = (100*len(training_x) // BATCH_SIZE))
...
results = model.fit(train_generator, steps_per_epoch=(len(training_x) // BATCH_SIZE), epochs=EPOCHS, validation_data=validation_generator, use_multiprocessing=False,workers=6,callbacks=[tensorboard, checkpoint, checkpoint2])
...
plt.plot(epochs,bin_acc,'-', label='categorical_accuracy')
plt.plot(epochs,val_bin_acc,'-',label='Validation categorical_accuracy')
plt.legend()
